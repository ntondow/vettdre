...
**Typecheck baseline (Phase 3 late, 2026-05-01):** 288 errors (measured on `feat/p3-6-emoji-to-lucide` branched off `origin/main` post-PR #28 merge, across two consecutive pre-edit runs and two consecutive post-edit runs — slice 9 itself adds zero). Was 286 at Phase 3 mid; the +2 regression accumulated silently on origin/main between PR #25 close and slice 9 measurement (so introduced by one of PRs #26, #27, or #28) — none of those PRs anchored the change at the time. Surfaced in PR #29 (slice 9) and re-anchored here per the surface-baseline-mismatches rule. Future slices must hold ≤288 or improve. **Measurement protocol:** because `git stash --include-untracked` will roll back tracked-file edits along with the dupes (learned in slice 9), prefer measuring against the dirty tree directly using `NODE_OPTIONS=--max-old-space-size=8192 npx tsc --noEmit 2>&1 | grep -E "error TS" | grep -vE "^[^(]* [0-9]+(\\.test)?\\.tsx?\\(" | wc -l` — the second grep filters Finder-dupe `.ts`/`.tsx` files (552 of 608 error-producing files on the laptop). For high confidence, run twice and require the same number; one-off readings can drift ±1 from cache state. Also: `tsc` OOMs on 4GB, so the `NODE_OPTIONS` heap bump is mandatory.
