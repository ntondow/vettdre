...
**Lint baseline (Phase 3 mid, 2026-05-01):** 4484 errors (measured on `main` post-PR #26 merge from a clean tree). Was 4520 at Phase 1 close (2026-04-30); the 36-error improvement accumulated silently across the Phase 2 + Phase 3 slices since the 4520 anchor was set — none of those PRs anchored the change at the time. Surfaced in PR #27 (slice 7) per the surface-baseline-mismatches rule and re-anchored in this PR's chore commit. Future slices must hold ≤4484 or improve; never let an in-flight phase silently increase it.

**Typecheck baseline (Phase 4, 2026-05-01):** 285 errors (measured on `main` post-PR #34 merge from a clean tree via `git stash --include-untracked`, two consecutive runs, stable). Was 288 at Phase 3 late; the -3 improvement = -2 silent drift on origin/main between slice 9 and slice 20-fixes-A measurement (introduced by one of PRs #30–#33, none anchored the change at the time) plus -1 from slice 20-fixes-A's signatureImage guard removing TS18048 on sign/route.ts:185. Surfaced in PR #34 (slice 20-fixes-A) and re-anchored here in slice 20-fixes-B per the surface-baseline-mismatches rule. Future slices must hold ≤285 or improve. **Measurement protocol:** because `git stash --include-untracked` will roll back tracked-file edits along with the dupes (learned in slice 9), prefer measuring against the dirty tree directly using `NODE_OPTIONS=--max-old-space-size=8192 npx tsc --noEmit 2>&1 | grep -E "error TS" | grep -vE "^[^(]* [0-9]+(\\.test)?\\.tsx?\\(" | wc -l` — the second grep filters Finder-dupe `.ts`/`.tsx` files (552 of 608 error-producing files on the laptop). For high confidence, run twice and require the same number; one-off readings can drift ±1 from cache state. Also: `tsc` OOMs on 4GB, so the `NODE_OPTIONS` heap bump is mandatory. **Note on dirty-tree drift:** even after dupe filtering, Finder dupes can pollute *canonical* test files (e.g. `src/lib/condo-ingest/building-signals.test.ts` flags 2 spurious "duplicate function implementation" errors when its 100+ Finder dupes are visible to tsc) — so dirty-tree measurements typically read +2 vs clean-tree. When in doubt, measure clean-tree via stash; the anchor of 285 is the clean-tree number.

**Typecheck baseline (Foundation Audit Z.0a, 2026-05-03):** 515 errors (measured by CI on `origin/main` via `.github/workflows/ci.yml` — `npx tsc --noEmit | grep -cE "error TS"`, single run captured 2026-05-03T14:18:05Z on PR #50). **This supersedes the 285 local-tree anchor as the source of truth.** Methodology v2.2 §"Verified-claim audit pattern" caught the drift on the first PR-blocking CI run that shipped — exactly the failure mode the v2.2 tightening was designed to surface. The 228-error gap (515 - 287 dirty-tree-filtered) is filed as Phase 5 stub `z0a-followup-typecheck-gap-investigation` in `SLICES-speed.md` for diff-and-categorize triage; three working hypotheses captured there. Going forward: **CI is the source of truth for baselines**; local measurements are estimates that may diverge due to Finder-dupe pollution, tooling drift (TS version via lockfile race, prisma generate output diff), or filter-regex incompleteness (the local filter only catches errors from dupe FILES, not dupe DIRECTORIES). The 285 anchor above is preserved as historical context — do NOT use it as the threshold for new CI checks.
