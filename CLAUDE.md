...
**Typecheck baseline (Phase 3 mid, 2026-05-01):** 286 errors (measured on `main` post-PR #25 merge across two consecutive runs from a clean tree). Was 292 at Phase 2 mid (2026-04-30); the 6-error improvement accumulated silently across PRs #21 (slice 18), #22 (slice 13), #23 (slice 6), #24 (Phase 3 slice 1 cleanup), #25 (slice 6-ext) — none of those PRs anchored the change at the time. Surfaced in PR #25 and re-anchored in PR #26 (slice 13-cross-cut) per the surface-baseline-mismatches rule. Future slices must hold ≤286 or improve. **Measurement protocol:** run `git stash --include-untracked` first to remove untracked Finder dupes from the laptop, then `npx tsc --noEmit 2>&1 | grep -cE "error TS"` — no `grep -v` filter (the filter regex used in slice 7a/17 PR descriptions was unreliable and produced fake "improvements"). For high confidence, run twice and require the same number; one-off readings can drift ±1 from cache state. The ~80+ untracked Finder-dupe files on Nathan's laptop inflate raw counts by ~200+ errors but never enter git.
