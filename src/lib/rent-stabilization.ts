...
// Now reads RGB rates from ReferenceDataCatalog with fallbacks
// ============================================================
...
function computeBlendedRate(rate: RGBRate): number {
  const twoYearAnnualized = ((1 + rate.twoYear / 100) ** 0.5 - 1) * 100;
  return rate.oneYear * 0.7 + twoYearAnnualized * 0.3;
}

/** Async version — reads latest RGB rate from DB, falls back to hardcoded */
export async function getBlendedRGBRateAsync(): Promise<number> {
  // Find the latest year in DB
  const currentYear = new Date().getFullYear();
  for (let year = currentYear; year >= currentYear - 2; year--) {
    const dbRate = await getReferenceData<RGBRate>("rgb_rates", String(year));
    if (dbRate) return computeBlendedRate(dbRate);
  }
  // Fallback to hardcoded
  return computeBlendedRate(RGB_RATES[0]);
}
