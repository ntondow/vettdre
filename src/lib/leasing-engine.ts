...
import { LEASING_GWS_TOOLS, executeGwsTool, isGwsTool } from "@/lib/gws-tools";
...
const TOOLS: Anthropic.Tool[] = [
  ...
  // Google Workspace tools (calendar, email confirmation, thread search)
  ...LEASING_GWS_TOOLS,
];
...
async function executeToolCall(
  ...
  switch (toolName) {
    ...
    default:
      // Check if it's a Google Workspace tool
      if (isGwsTool(toolName)) {
        try {
          return await executeGwsTool(toolName, input, {
            orgId: config.orgId,
            userId: config.userId || undefined,
          });
        } catch (err: any) {
          console.error(`[Leasing] GWS tool ${toolName} failed:`, err.message);
          return `Google Workspace tool error: ${err.message}`;
        }
      }
