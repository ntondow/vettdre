...
// Now reads FHFA metro data from ReferenceDataCatalog with fallbacks.
...
// Map borough codes / zip prefixes to FHFA metro key
function getMetroKey(zip: string): string {
  const prefix = zip.slice(0, 3);
  if (prefix >= "070" && prefix <= "079") return "Newark";
  if (prefix >= "110" && prefix <= "119") return "Nassau";
  return "NYC";
}

// Sync version — hardcoded only (for cache fallback path)
function getMetro(zip: string): typeof FHFA_METRO[string] {
  return FHFA_METRO[getMetroKey(zip)];
}

// Async version — tries DB first, falls back to hardcoded
async function getMetroAsync(zip: string): Promise<typeof FHFA_METRO[string]> {
  const key = getMetroKey(zip);
  const dbData = await getReferenceData<typeof FHFA_METRO[string]>("fhfa_hpi", key);
  return dbData ?? FHFA_METRO[key];
}
...
export async function getMarketAppreciation(zip: string): Promise<MarketAppreciation> {
  ...
  const metro = await getMetroAsync(cleanZip);
