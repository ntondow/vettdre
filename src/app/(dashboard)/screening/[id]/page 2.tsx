...
export default function ScreeningDetailPage() {
  ...
  const primaryApplicant = app.applicants?.find((a: any) => a.role === "primary");
  const otherApplicants = app.applicants?.filter((a: any) => a.role !== "primary") || [];
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-1.5">
              ...
              <span className="text-sm text-slate-500">
                {app.tier === "enhanced" ? "Enhanced" : "Base"} Screening
              </span>
      ...
      {/* Score + Decision Card */}
      {(app.riskScore != null || app.status === "complete") && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white rounded-lg border border-slate-200 p-5">
            <h3 className="text-sm font-semibold text-slate-700 mb-3">VettdRE Risk Score</h3>
            <div className="flex items-center gap-4">
              <RiskScoreBadge score={app.riskScore} recommendation={app.recommendation} size="lg" />
              {app.recommendation && (
                <div>
                  <p className="text-sm text-slate-500">AI Recommendation</p>
                  <p className="text-sm font-semibold capitalize text-slate-700">{app.recommendation}</p>
                </div>
              )}
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm mt-3">
          <div>
            ...
            <span className={`text-xs font-medium px-2 py-0.5 rounded ${
              app.tier === "enhanced" ? "bg-violet-100 text-violet-700" : "bg-slate-100 text-slate-600"
            }`}>
              {app.tier === "enhanced" ? "Enhanced" : "Base"}
            </span>
          </div>
          <div>
