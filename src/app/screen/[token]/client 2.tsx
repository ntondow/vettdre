...
export default function ScreeningWizardClient({ token }: { token: string }) {
  ...
  const primaryApplicant = data?.applicants?.find((a) => a.role === "primary");
  ...
  const handleDocumentsComplete = async (documents: Array<{ file: File; documentType: string }>) => {
    ...
    try {
      setSaving(true);
      // TODO: Upload files to Supabase Storage first, then save metadata
      // For now, save metadata with placeholder URLs
      const docMeta = documents.map((d) => ({
        fileName: d.file.name,
        filePath: `pending-upload/${d.file.name}`,
        documentType: d.documentType,
        fileSize: d.file.size,
        mimeType: d.file.type,
      }));
      await apiPost("/documents", { applicantId, documents: docMeta });
      setStep("payment");
    } catch (e: any) {
      setError(e.message);
    } finally {
      ...
      >
        {step === "personal_info" && (
          <PersonalInfoStep
            ...
            applicantRole={primaryApplicant?.role || "primary"}
