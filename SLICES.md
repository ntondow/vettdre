...
### 19-fix-tra-seed-coords — TRA template field positions + missing agent block
- **Status:** `awaiting_review`
- **Goal:** Fix the seeded Tenant Representation Agreement template's Page 2 field positions (overlay boxes hovered ~15% below the underlines users were supposed to fill) AND add the missing agent-block fields (3 underlines for AGENT Printed Name / Signature / Date were drawn by the generator but had no template entries — agents had no signable affordance at all). Structural fix on both sides: generator now exports `fieldCoords` derived from the actual `drawBlankLine` y-positions; seed builds the field array from those coords. One-shot migration script back-fills already-seeded prod rows with an exact-match guard so manager customizations made via slice 19-B2b drag/resize are preserved.
- **Bug origin:** Hand-tuned percentages in `onboarding-seed-templates.ts` (`{ x: 18, y: 69 }` / `{ x: 18, y: 75 }` / `{ x: 18, y: 83 }` for the 3 tenant fields) drifted from the generator's actual underline positions (~54% / 57% / 60%). Pre-fix mental model assumed a static government PDF for TRA — discovery surfaced that the TRA is generated programmatically via pdf-lib with NO AcroForm widgets, so `pdfjs.getAnnotations()` couldn't extract coords. The fix had to make the generator the geometry authority instead.
- **Files:**
  - `src/lib/onboarding-pdf.ts` — exports `TraFieldCoord` + `TraFieldCoords` interfaces; generator return changes from `Promise<Uint8Array>` to `Promise<{ pdfBytes; fieldCoords: TraFieldCoords }>`; captures y-position before each of the 6 `drawBlankLine` calls on page 2 and converts to top-percentage box coords (box bottom = underline so users sign on top of the line); adds `topPctFromBaseline()` helper + 6 geometry constants (`TRA_FIELD_X`, etc.).
  - `src/lib/onboarding-seed-templates.ts` — removes hardcoded `TENANT_REP_FIELDS` array; adds `buildTraFieldsFromCoords()` exported helper yielding 6 fields (3 tenant + 3 agent) with metadata (id, label, type, prefillKey, required) destructure-spread over coords; refactors `TemplateConfig` from `{ fields, generatePdf }` → `{ build }` returning `{ pdfBytes, fields }` so each config owns its own derivation.
  - `src/app/(dashboard)/brokerage/client-onboarding/actions.ts` — fallback non-template path destructures `{ pdfBytes }` from new generator return.
  - `scripts/migrate-tra-template-fields.ts` (new) — exact-match guard on `(x:18, y:69)` / `(x:18, y:75)` / `(x:18, y:83)`; dry-run default ON; `--apply` to write; `--org-id <id>` filter; JSON backup snapshot before any DB or Storage writes; per-row PDF regen + re-upload on matched rows only; skip-with-log for non-matches preserving manager customizations.
  - `tests/smoke/tra-template-coords.test.ts` (new — 5 contracts): C1 generator exports `TraFieldCoords` + new return shape (with negative pin on legacy `Promise<Uint8Array>`); C2 seed has no `(x:18, y:69|75|83)` pairs (scoped to that combination because NYS legitimately uses bare `y: 83` at `x: 5`); C3 seed imports `TraFieldCoords` and pipes destructured coords through `buildTraFieldsFromCoords`; C4 builder yields 6 fields including all 3 agent ids (locks the agent-block fix structurally); C5 migration has known-bad-coords guard with all 3 exact y-pins + dry-run default + JSON backup.
- **Success criteria:** typecheck holds at clean-tree 285 (= dirty-tree 287); lint changed-files only — zero new errors; build passes; full vitest suite passes (306/306); 5 smoke contracts green; visual check post-merge in dev confirms overlays sit on the underlines.
- **Migration deploy plan:** Nathan-driven only. After PR merges + main deploys: dry-run with `--org-id <pilot org>` first → verify backup JSON + console output → live `--apply --org-id <pilot org>` → manual visual check on `/brokerage/client-onboarding/vault/[id]` → repeat for remaining orgs (or unscoped `--apply` once confidence is high). Backup JSON is the rollback path if anything looks wrong.
- **Depends on:** 19-B2b (merged) — drag/resize UI is the recovery path for any TRA template that the migration's exact-match guard skips.
- **Requires approval:** Pre-approved by Nathan (6 fields scope, exact-match guard, dry-run default ON, JSON backup, per-row PDF regen, +4th smoke contract).
- **Outcome:** Merged + migrated 2026-05-02 (PR #41, Gulino's TRA template `cmoiwqbtp0001e8cl9cqrfb3c` updated 12:53Z). Visual inspection surfaced a follow-up: signature boxes at height 7% overlapped the printed-name field above them — fixed in slice 19-fix-tra-sig-height (below).

### 19-fix-tra-sig-height — TRA signature box height + signing UI tap-target carve-out
- **Status:** `awaiting_review`
- **Goal:** Fix the click-block / visual overlap where TRA Page 2 signature boxes covered the printed-name field above them. Two-layer fix: shrink `TRA_SIG_HEIGHT` 7→3 in the generator (fixes the vault editor where `field.height` is used directly), AND add a signature carve-out in `pdf-field-viewer.tsx` so the inline overlay uses a 24px floor (WCAG 2.5.8 AA) instead of 44px (WCAG 2.5.5 AAA / iOS HIG) on the signing UI — needed because the 44px minHeight floor wins over `field.height` on small displays, so shrinking the constant alone wouldn't propagate to /sign/[token]. Justification for dropping AAA→AA: tapping a signature opens a full-screen modal pad — the inline overlay only needs to be tappable to OPEN the pad, not be the actual signing surface.
- **Closes bug:** Visual overlap surfaced post-migration on Gulino's TRA template (PR #41). Tenant signature box at top 49.82% / height 7% engulfed Tenant Printed Name box at top 50.04% / height 4%. Same overlap on agent block. Per CSS painting rules, later DOM siblings paint on top of earlier ones without explicit z-index — printed-name field had zero unique clickable area at signing time.
- **Files:**
  - `src/lib/onboarding-pdf.ts` — `TRA_SIG_HEIGHT` constant 7 → 3. With underline cascade ~2.78% per row, 3% leaves ~0.22% of vertical share with text fields below — visually negligible, no overlap on the vault editor.
  - `src/components/onboarding/pdf-field-viewer.tsx` — outer signature ternary on `minHeight` + `minWidth`: `field.type === "signature" ? "24px" : (field.prefillKey ? "16px" : "44px")`. Comment explains the WCAG 2.5.8 AA / 2.5.5 AAA trade and the modal-pad reasoning.
  - `tests/smoke/signing-fixes-C.test.ts` — relaxed slice 20-C #13 regex to tolerate the optional outer signature ternary while still pinning the inner `prefill ? "16px" : "44px"` branch. Spirit of HIG #13 preserved (44px for non-signature interactive fields); regex shape evolved to match the new ternary nesting. Title updated to "non-prefill, non-signature" for clarity.
  - `tests/smoke/tra-template-coords.test.ts` — adds C6 (`TRA_SIG_HEIGHT` < 5 in generator — regression guard) and C7 (`pdf-field-viewer.tsx` differentiates signature minHeight at 24px from non-signature; negative pin on flat 44px floor).
  - `scripts/migrate-tra-sig-heights.ts` (new) — match guard: any field with `type: "signature" && height === 7`. Dry-run default ON. `--apply` for writes. `--org-id <id>` filter. JSON backup snapshot before any writes. Per-row PDF regen + re-upload on matched rows only.
- **Success criteria:** typecheck holds at clean-tree 285 (=dirty-tree 287); full vitest suite passes (308/308); slice 20-C #13 contract still passes after regex relaxation; build clean; lint changed-files clean; visual checks confirm signature overlay no longer engulfs printed-name on both vault editor + signing UI.
- **Migration deploy plan:** Nathan-driven only. Expected match in prod: exactly 1 row (Gulino's `cmoiwqbtp0001e8cl9cqrfb3c`). Dry-run unscoped first → verify match count = 1 + backup JSON → `--apply --org-id 5ecba9ba-6de1-4b1e-bb6a-3f2dfef81670` → visual check on `/brokerage/client-onboarding/vault/[id]` and `/sign/[token]` for a Gulino test onboarding. If dry-run reports >1 matches, stop and investigate.
- **Depends on:** 19-fix-tra-seed-coords (merged + migrated, PR #41).
