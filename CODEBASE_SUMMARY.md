...
Automations Engine (MVP) — Rule-based workflow automation triggered by CRM events. 4 lib files: automation-types.ts (trigger configs, conditions with 7 operators + AND/OR groups, 4 action types), automation-evaluator.ts (condition evaluation), automation-executor.ts (create_task with template tokens, update_contact_status, send_notification, add_tag), automation-dispatcher.ts (safe fire-and-forget dispatch). 5 trigger types: new_lead, stage_change, no_activity, task_overdue, showing_completed. Fire points in contacts/actions.ts, brokerage/transactions/actions.ts, calendar/actions.ts. Cron endpoint /api/automations/cron (Bearer auth, checks no_activity + task_overdue, batch limit 100). Settings UI at /settings/automations: CRUD with inline expand-to-edit, conditions builder, actions builder, active toggle, stats bar, empty state. AutomationRun audit trail per execution.
My Properties Unified Hub — Replaced /properties redirect with full client page aggregating 4 data sources (BMS Listings, CRM Deals, Showings, Prospecting Items) via Promise.allSettled(). Source-prefixed IDs (lst_, deal_, shw_, prs_), address-based deduplication, client-side filtering (debounced search, source pills, sort dropdown). Property cards with source/status badges, specs, deal/showing counts. Stats bar with clickable source filter. New "Portfolio" sidebar section.
...
| `/analytics` | Not built |
| Properties page | ✅ Complete (unified hub: BMS Listings + CRM Deals + Showings + Prospecting) |
...
| Automations | ✅ MVP Complete (engine + cron + UI; Phase 2: send_email, templates, run history) |
